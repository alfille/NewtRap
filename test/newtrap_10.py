...
class _slope:
    def __init__(self):
        # keep last 10 slopes (would be easier with numpy but don't want dependency)
        self.size = 10
        self.slot = 0
        self.sign = 1
        self.slopes = [ 1 for i in range(self.size) ] # initial
    @property
    def deriv(self):
        a = self.sign * sum( self.slopes ) / self.size
        self.sign = -self.sign
        if a == 0:
            return 1
        return a
...
#        self.chain = _iir( value = .5, chain = self.chain )
    ...
    def new_pair( self ):
        ...
        if dx == 0 or dy == 0:
            ...
            if abs(self.ypair0) < abs(self.ypair1):
                x2 = self.xpair0 - self.ypair0/ self.slope.deriv
            else:
                x2 = self.xpair1 - self.ypair1/ self.slope.deriv

        else:
            deriv = dy / dx
            self.slope.deriv = deriv
    ...
    def target( self, t ):
        # Big jostle
        self.xpair0 += (t - self._target) * self.slope.deriv # deriv is essentially conversion factor
        # New target
        self._target = t
        self.new_settings()
